    ...
    Object.entries(daerahMap).forEach(([daerah, entries]) => {
      ...
      entries.forEach((entry) => {
        ...
        if (lowPower || lowMER || lowCN || lowLinkMargin) {
          ...
          if (lowLinkMargin) kecamatanMap[entry.kecamatan].add('Link Margin rendah');
        }
      });
...
\n`;
  ...
  const linkMarginStats = {
    ...
    cukup: data.filter(item => item.linkMargin >= 3 && item.linkMargin <= 6).length,
  ...
  labels: ['Baik (>48 dBµV/m)', 'cukup (45–48 dBµV/m)', 'Rendah (<45 dBµV/m)'],
  datasets: [{
    label: 'Jumlah Data',
  ...
  const linkMarginChartData = {
    ...
    datasets: [{
      label: 'Jumlah Data',
      data: [linkMarginStats.tinggi, linkMarginStats.cukup, linkMarginStats.rendah],
      backgroundColor: function(context) {
      const index = context.dataIndex;
      return ['#16a34a', '#2563eb', '#dc2626'][index]; // gradien dasar bisa dipakai nanti
    },
  ...
  if (loading) {
    ...
    return (
      <div className="dashboard-home">
        ...
        {/* Grafik */}
        <div className="charts-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px', marginTop: '20px' }}>
        ...
        <div className="dashboard-row" style={{ marginTop: '20px', display: 'flex', gap: '20px' }}>
                  ...
                  {latestData.map((item, index) => {
                    ...
                    return (
                      <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
                        ...
                        <td style={{ padding: '12px' }}>{item.power.toFixed(1)}</td>
                        <td style={{ padding: '12px' }}>{item.cn.toFixed(1)}</td>
                        <td style={{ padding: '12px' }}>{item.mer.toFixed(1)}</td>
                        <td style={{ padding: '12px' }}>{item.linkMargin.toFixed(1)}</td>
    ...
    insights.map((item, index) => (
      <div
        ...
        style={{
          ...
          borderLeft: `6px solid ${item.type === 'danger' ? '#dc2626' : '#eab308'}`,
          borderRadius: '14px',
          padding: '24px',
          marginBottom: '24px',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px' }}>
          <AlertCircle size={26} style={{ color: item.type === 'danger' ? '#dc2626' : '#ca8a04', marginRight: '10px' }} />
          <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#111827' }}>
            Wilayah: <span style={{ color: '#1d4ed8' }}>{item.daerah}</span>
          </h3>
        </div>
