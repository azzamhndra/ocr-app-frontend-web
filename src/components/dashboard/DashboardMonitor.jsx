...
function DashboardMonitor() {
  ...
  function getSignalStatus(power, cn, mer, linkMargin) {
    if (power >= 45 && cn >= 20.5 && mer >= 26 && linkMargin >= 3) {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://api.ocrapp.biz.id/data");
      if (!response.ok) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://api.ocrapp.biz.id/data/${selectedUser.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const exportToExcel = (groupBy = "daerah") => {
    ...
    users.forEach((item) => {
      const key =
        groupBy === "status"
          ? getSignalStatus(item.power, item.cn, item.mer, item.linkMargin)
              .label
          : item.daerah;
      ...
      groupedData[key].push({
        ...
        Latitude: item.lat,
        Longitude: item.lon,
        Status: getSignalStatus(item.power, item.cn, item.mer, item.linkMargin)
          .label,
      });
    });
              ...
              <div className="user-details">
                <div className="user-info-grid">
                  ...
                  <div className="info-group">
                    ...
                    <p>
                      Lat: {selectedUser.lat}, Lon: {selectedUser.lon}
                    </p>
                  </div>
                  <div className="info-group">
                    <label>Status</label>
          ...
          <div className="modal-container">
            ...
            <div className="modal-body">
              <div className="delete-confirmation">
                ...
                <p>
                  Are you sure you want to delete this data from
                  <br /> <strong>{selectedUser.daerah}</strong>?
                </p>
                <p
